...
async function init() {
    ...
    points.forEach((pos, index) => {
        ...
        gsap.set([img, shadow], { xPercent: -50, yPercent: -50 });
        gsap.set(wrapper, { xPercent: -50, yPercent: -50, scale: 2, opacity: 0 });

        // Начальное состояние для GSAP
        gsap.set(shadow, {
            x: 368 * SHADOW_RATIO,
            y: 258 * SHADOW_RATIO,
            filter: `blur(${50 * SHADOW_RATIO}px)`,
            opacity: 0.5
        });
...
function showSquare(index) {
    ...
    gsap.to(img, {
        rotation: randomRotation, // Картинка крутится
        duration: 0.6
    });
